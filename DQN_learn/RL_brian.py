...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        self.a = tf.placeholder(tf.int32, [None, ], name='a')  # input Action # input Action
        ...
        with tf.variable_scope('q_target'):
            ...
            # 使用stop_gradient将不会影响target-net的参数，相当于把target-net的参数拷贝一份用于计算误差
            self.q_target = tf.stop_gradient(q_target)

        with tf.variable_scope('q_eval'):
            # a是batch_memory feed过来的使用了的动作的集合
            # 使用range生成相应state的序号，并用tf.stack进行组合
            # 从q_eval中找出组合好的对应的值
